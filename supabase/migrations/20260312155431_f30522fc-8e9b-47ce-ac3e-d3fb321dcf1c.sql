...
AS $$
  SELECT
    ...
    coalesce(sum(s.diferenca_valor), 0) AS economia,
    CASE WHEN coalesce(sum(s.valor_cobrado), 0) = 0 THEN 0
         ELSE round(coalesce(sum(s.diferenca_valor), 0) / sum(s.valor_cobrado) * 100, 1)
    END AS pct_economia
  FROM simulations s
  GROUP BY s.study_id;
$$;